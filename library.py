...
def add_new_book():
    book_title = input("Enter the book's title: ") # Get book title
    book_author = input("Enter the book's author: ") # Get book author
    book_status = input("Have you read this? (Y/N)") # Get read or tbr status
    book_tropes_string = input("What are the tropes? (each new trope separated by '|' with no spaces) ") # Get book tropes
...
def library_lobby():
    ...
    while True: # This loop keeps the program running until explicitly told to stop
        ...
        if choice == '1':
            ...
            print("You chose to view your TBR list!")
